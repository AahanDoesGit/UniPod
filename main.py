...
pygame 240×320, 30 FPS, non-blocking loop.
"""
...
def main():
    ...
    is_unihiker = os.path.exists("/sys/class/gpio")
    flags = pygame.FULLSCREEN if is_unihiker else 0
    display = pygame.display.set_mode((SCREEN_W, SCREEN_H), flags)
    ...
    music_dir = "/root/music" if is_unihiker else "./music"
    library.load_from_directory(music_dir)

    boot_scr  = BootScreen()
    menu_scr  = MainMenuScreen(header, listview, wheel)
    music_scr = MusicListScreen(header, listview, wheel, library, audio)
    play_scr  = NowPlayingScreen(wheel, library, audio)
